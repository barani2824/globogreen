...
export class UserManagementComponent implements OnInit, OnDestroy {
  ...
  downloadForm(user: User): void {
    this.userService.getPDF(user.email).subscribe(
      (res: Blob) => {
        this.download(res, user?.firstName);
      },
      err => {
        console.error('Error while Downloding Basic Form', err);
      }
    );
  }

  download(data: Blob, filename: string | undefined): void {
    const downloadURL = window.URL.createObjectURL(data);
    const link = document.createElement('a');
    link.href = downloadURL;
    link.download = (!filename ? 'form' : filename) + '.pdf';
    link.click();
  }
